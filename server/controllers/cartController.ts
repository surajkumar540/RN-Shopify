...
export const updateToCartItem = async (req: Request, res: Response) => {
  try {
...
export const removeToCartItem = async (req: Request, res: Response) => {
  try {
    ...
    // ✅ added return + negated logic
    cart.items = cart.items.filter((item) => {
      return !(item.product.toString() === req.params.productId && item.size === size);
    });
